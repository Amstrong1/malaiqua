<div class="flex flex-col items-stretch w-full overflow-hidden rounded shadow-xs border">
        ...
        <div class="flex justify-center items-center p-4 border-b table-search-container">
            ...
            <div class="relative">
                ...
                <input type="text" id="custom-search-input"
                    class="block p-2 pl-10 w-80 text-sm text-gray-900 bg-white rounded border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                                ...
                                <p class="font-semibold capitalize">{{ $resource->name }}</p>
                                    ...
                                    @elseif ($column == 'status')
                                        <span @class([
                                            ...
                                            'text-green-700 bg-green-100' => $resource->$column == 'Terminé',
                                            ' text-gray-700 bg-gray-100' => $resource->$column == 'En cours',
                                            ' text-gray-700 bg-yellow-100' => $resource->$column == 'En attente',
                                        ])>
                                            {{ $resource->{$column} }}
                                        </span>
                                    @else
                                        ...
                                        @foreach ($mactions as $action => $title)
                                            @if ($action == 'show')
                                                <a href="{{ route($type . '.show', [$type => $resource->id]) }}"
                                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 rounded focus:outline-none focus:shadow-outline-gray">
                                            ...
                                            @elseif ($action == 'edit')
                                                <a href="{{ route($type . '.edit', [$type => $resource->id]) }}"
                                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-main rounded focus:outline-none focus:shadow-outline-gray"
                                                    aria-label="Edit">
                                            ...
                                            @elseif ($action == 'delete')
                                                <form action="{{ route($type . '.destroy', [$type => $resource]) }}"
                                                    ...
                                                    @csrf
                                                    <button type="submit"
                                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-red-600  rounded focus:outline-none focus:shadow-outline-gray"
                                                        aria-label="Delete">
                                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
                                                            viewBox="0 0 20 20">
                                                            <path fill-rule="evenodd"
